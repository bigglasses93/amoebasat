...
int main(int argc, char** argv) {
    ...
    x = (bool*) malloc(N_VARIABLE+1); 
    x_fixed = (bool*) malloc(N_VARIABLE+1);

    LargeX=(int**)malloc(sizeof(int*) * (N_VARIABLE+1));
    for(int i=0;i<= N_VARIABLE;i++){
        LargeX[i]=(int*)malloc(sizeof(int) * 2);
    }

    Y=(bool**)malloc(sizeof(bool*) * (N_VARIABLE+1));
    for(int i=0;i<= N_VARIABLE;i++){
        Y[i]=(bool*)malloc(sizeof(bool) * 2);
    }
    L=(bool**)malloc(sizeof(bool*) * (N_VARIABLE+1));
    for(int i=0;i<= N_VARIABLE;i++){
        L[i]=(bool*)malloc(sizeof(bool) * 2);
    }
    Z=(int**)malloc(sizeof(int*) * (N_VARIABLE+1));
    for(int i=0;i<= N_VARIABLE;i++){
        Z[i]=(int*)malloc(sizeof(int) * 2);
    }
